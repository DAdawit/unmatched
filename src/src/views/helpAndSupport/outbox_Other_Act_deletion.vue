    ...
    <div>
        ...
        <div v-for="(out,index) in outbox" :key="index">
            <v-card width="100%" outlined>
                ...
                <v-list>
                    <v-list-item :to="{name:'other', params:{id:out.id}}">
                        <v-list-item-avatar>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapGetters({
                accountDeleteMessages: 'compose/accountDeleteMessages',
                user: 'auth/user'
            })
        },
        methods: {
            ...mapActions({
                GetDeleteAccounts: 'compose/GetDeleteAccounts'
            })
        },
        async created() {
            this.GetDeleteAccounts().then(() => {
                this.outbox = this.accountDeleteMessages.results.filter((item) => {
