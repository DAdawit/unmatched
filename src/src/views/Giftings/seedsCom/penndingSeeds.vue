<template>
    <div>
        ...
        <br> <br><br>
        <v-container v-if="pendingSeedLength >0">
            <v-row>
                <h1 class="text-underline text--secondary">Pennding Seed list</h1>
            </v-row>
            <v-card text v-for="seed in pendingseeds" :key="seed.id" :class="` post ${seed.status} py-2 px-5 mt-5`">
                <v-container>
                    <v-layout row wrap class="pa-3">
                        ...
                        <v-flex xs6 sm4 md2 lg2>
                            ...
                            <!-- :to="{name:'editSeed', params:{id:seed.id}}" -->
                            <v-btn small icon color="orange" :to="{name:'editSeed', params:{id:seed.id}}">
                                <v-icon>edit</v-icon>
                            </v-btn>
        ...
        <v-card class="mx-3" v-if="pendingSeedLength < 1" flat tile>
            <v-card-title class="d-flex justify-center">No Pending seed data</v-card-title>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            editSeed(seed) {
                // this.dialog=true;
                this.editData.amount = seed.amount;
                this.editData.description = seed.description;
                this.editData.round = seed.round
        ...
        mounted() {
            this.pendingseeds = this.seeds.results.filter((item) => {
                return item.status.match('Pending')
            })
            this.pendingSeedLength = this.pendingseeds.length;
